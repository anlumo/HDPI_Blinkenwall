use glium::{Surface, Frame, uniform};
...
use palette::{GammaSrgba, RgbHue, Alpha};
use std::borrow::Cow;
use rand::Rng;
use std::{cmp,u8};
...
impl Poem {
    pub fn new<R: Rng>(display: &Display, font: &bdf::Font, text: &str, rand: &mut R) -> Poem {
        ...
        let width = cmp::min(window_size.width as usize / char_size.0, text.lines().map(|line| line.len()).max().unwrap());
        let height = cmp::min(window_size.height as usize / char_size.1, text.lines().count());
        ...
        Poem {
            created: Instant::now(),
            color: GammaSrgba::from(Alpha::hsv(RgbHue::from_degrees(rand.gen_range(0, 36000) as f32 / 100.0), 1.0, 1.0, 1.0)),
            texture: texture,
    ...
    pub fn render_all(display: &Display, poems: &Vec<Poem>, vertex_buffer: &glium::VertexBuffer<Vertex>, index_buffer: &glium::IndexBuffer<u16>, program: &glium::Program) {
        ...
        for poem in poems {
            poem.render(&mut target, &(size.width as u32, size.height as u32), &vertex_buffer, &index_buffer, &program);
        }
